...
const categoryConfig = {
  plumbers: { name: 'Plumbers', icon: Droplets, color: 'from-blue-500 to-cyan-500', apiProfession: 'plumber', description: 'Expert plumbers for all your pipe and water needs' },
  electricians: { name: 'Electricians', icon: Zap, color: 'from-yellow-500 to-orange-500', apiProfession: 'electrician', description: 'Certified electricians for safe and reliable electrical work' },
  handymen: { name: 'Handymen', icon: Hammer, color: 'from-purple-500 to-pink-500', apiProfession: 'handyman', description: 'Skilled handymen ready for any home repair or maintenance task' },
  'furniture-assembly': { name: 'Furniture Assembly', icon: Armchair, color: 'from-green-500 to-emerald-500', apiProfession: 'furniture assembly', description: 'Professional furniture assembly and installation specialists' },
  'air-conditioning': { name: 'Air Conditioning', icon: Wind, color: 'from-cyan-500 to-blue-500', apiProfession: 'air conditioning', description: 'AC experts for installation, repair, and maintenance' },
  painters: { name: 'Painters', icon: PaintBucket, color: 'from-red-500 to-pink-500', apiProfession: 'painter', description: 'Skilled painters for beautiful interior and exterior finishes' },
  'tv-mounting': { name: 'TV Mounting', icon: Tv, color: 'from-indigo-500 to-purple-500', apiProfession: 'tv mounting', description: 'Professional TV mounting and home theater installation' },
  'auto-mechanics': { name: 'Auto Mechanics', icon: Car, color: 'from-gray-600 to-gray-800', apiProfession: 'auto mechanic', description: 'Trusted mechanics for all vehicle repairs and maintenance' },
  'general-help': { name: 'General Help', icon: Wrench, color: 'from-blue-600 to-indigo-600', apiProfession: 'general help', description: 'Versatile helpers for various home and office tasks' }
};
...
const HireModal = ({ craftsman, onClose, navigate }) => {
  ...
  // Čekamo dok AuthContext ne učita usera iz tokena
  ...
  const handleBooking = async (e) => {
    ...
    if (!userId || isNaN(Number(userId))) {
      setBookingError('Vaša sesija je istekla. Molimo prijavite se ponovo.');
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold text-lg">{craftsman.firstName} {craftsman.lastName}</h2>
        ...
        <div className="flex border-b border-gray-700">
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition ${tab === 'profile' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-white'}`}
          >
            Profil
          </button>
          <button
            onClick={() => setTab('book')}
            className={`flex-1 py-3 text-sm font-medium transition ${tab === 'book' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-white'}`}
          >
            Zakaži posao
          </button>
        </div>
          ...
          <div className="p-5 space-y-5">
            <div className="flex items-center gap-3">
              {craftsman.averageRating != null ? (
                <>
                  ...
                  <span className="text-white font-bold text-lg">{craftsman.averageRating.toFixed(1)}</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-800 rounded-xl p-4">
                ...
                <p className="text-white text-sm font-medium">{craftsman.location || 'N/A'}</p>
              </div>
              <div className="bg-gray-800 rounded-xl p-4">
                <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  <Briefcase className="w-3.5 h-3.5" /> Iskustvo
                </div>
                <p className="text-white text-sm font-medium">{craftsman.experience} god.</p>
              </div>
              <div className="bg-gray-800 rounded-xl p-4">
                <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  <Clock className="w-3.5 h-3.5" /> Radno vreme
                </div>
                <p className="text-white text-sm font-medium">{craftsman.workingHours || 'N/A'}</p>
              </div>
              <div className="bg-gray-800 rounded-xl p-4">
                <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                  <DollarSign className="w-3.5 h-3.5" /> Cena/sat
                </div>
                <p className="text-white text-sm font-bold">{craftsman.hourlyRate} RSD</p>
              </div>
            </div>

            <div className="bg-gray-800 rounded-xl p-4 space-y-3">
              <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Kontakt</p>
              <a href={`mailto:${craftsman.email}`} className="flex items-center gap-3 text-gray-300 hover:text-blue-400 transition text-sm">
                <Mail className="w-4 h-4 text-gray-500" />
                {craftsman.email}
              </a>
              <a href={`tel:${craftsman.phone}`} className="flex items-center gap-3 text-gray-300 hover:text-blue-400 transition text-sm">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleChat}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white rounded-xl transition text-sm font-medium"
              >
              ...
              <button
                onClick={() => setTab('book')}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition text-sm font-semibold"
              >
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              {!isLoggedIn && (
                <div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg flex items-center gap-2 text-yellow-400 text-sm">
                  ...
                  Morate biti prijavljeni da biste zakazali posao.
                </div>
              )}
              ...
              <div>
                <label className="block text-gray-300 text-sm font-medium mb-2">Datum i vreme</label>
                <input
                  type="datetime-local"
                  required
                  value={booking.scheduledDate}
                  onChange={e => setBooking({ ...booking, scheduledDate: e.target.value })}
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                />
              </div>

              <div>
                <label className="block text-gray-300 text-sm font-medium mb-2">Opis posla</label>
                <textarea
                  required
                  value={booking.jobDescription}
                  onChange={e => setBooking({ ...booking, jobDescription: e.target.value })}
                  rows={3}
                  placeholder="Opišite šta je potrebno uraditi..."
                  className="w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm resize-none"
                />
              </div>

              <div>
                <label className="block text-gray-300 text-sm font-medium mb-2">
                  Procenjeno sati: <span className="text-blue-400">{booking.estimatedHours}h</span>
                </label>
                <input
                  type="range"
                  min="1" max="12" step="1"
                  value={booking.estimatedHours}
                  onChange={e => setBooking({ ...booking, estimatedHours: parseInt(e.target.value) })}
              ...
              <div className="flex items-center gap-3 bg-gray-800 rounded-xl p-4">
                ...
                <label htmlFor="urgent" className="text-gray-300 text-sm cursor-pointer">
                  <span className="font-medium">Hitno</span>
                  <span className="text-gray-500 ml-1">— potrebna brza intervencija</span>
                </label>
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
  ...
  return (
    <div className="group bg-gray-800/60 backdrop-blur-sm rounded-2xl border border-gray-700 hover:border-blue-500/60 transition-all duration-300 overflow-hidden hover:shadow-xl hover:shadow-blue-900/20 hover:-translate-y-1">
      ...
      <div className="p-6 pt-4 space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <div>
            ...
            <span className="text-gray-400 text-sm ml-1">RSD/hr</span>
          </div>
          <button
            onClick={() => onHire(craftsman)}
      ...
      <div className="px-6 pb-5 flex gap-3">
        ...
        <a href={`tel:${craftsman.phone}`} className="flex items-center gap-1.5 text-gray-400 hover:text-blue-400 transition-colors text-xs">
          <Phone className="w-3.5 h-3.5" />
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-8">
              ...
              {[
                ...
                <button key={opt.value} onClick={() => setSortBy(opt.value)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${sortBy === opt.value ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700 border border-gray-700'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          {error && (
            <div className="flex flex-col items-center justify-center py-32 gap-4">
              ...
              <button onClick={() => window.location.reload()} className="mt-2 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors text-sm">
                Try again
              </button>
            </div>
          )}
          ...
          {!loading && !error && craftsmen.length === 0 && (
            <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
              ...
              <p className="text-gray-400 max-w-md">There are currently no {config.name.toLowerCase()} registered.</p>
              <Link to="/browse-tasks" className="mt-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors font-semibold">
