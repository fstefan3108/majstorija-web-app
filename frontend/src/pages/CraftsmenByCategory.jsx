  ...
  return (
    <div className="group bg-gray-800/60 backdrop-blur-sm rounded-2xl border border-gray-700 hover:border-blue-500/60 transition-all duration-300 overflow-hidden hover:shadow-xl hover:shadow-blue-900/20 hover:-translate-y-1">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              {craftsman.averageRating != null ? (<><StarRating rating={craftsman.averageRating} /><span className="text-gray-400 text-sm">{craftsman.averageRating.toFixed(1)}</span></>) : <span className="text-gray-500 text-sm">No reviews yet</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 pt-4 space-y-3">
        ...
        <div className="flex items-center gap-2 text-gray-300 text-sm"><Briefcase className="w-4 h-4 text-gray-500 flex-shrink-0" /><span>{craftsman.experience} year{craftsman.experience !== 1 ? 's' : ''} of experience</span></div>
        ...
        <div className="flex items-center justify-between pt-2">
          <div><span className="text-2xl font-bold text-white">{craftsman.hourlyRate}</span><span className="text-gray-400 text-sm ml-1">RSD/hr</span></div>
          <button onClick={() => onHire(craftsman)} className="flex items-center gap-1 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-all hover:shadow-lg hover:shadow-blue-600/30">Hire <ChevronRight className="w-4 h-4" /></button>
        </div>
      </div>
      <div className="px-6 pb-5 flex gap-3">
        <Link to={`/craftsman/${craftsman.craftsmanId}`} className="flex-1 flex items-center justify-center gap-2 bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium px-4 py-2 rounded-lg transition-all border border-gray-600"><User className="w-4 h-4" />View Profile</Link>
            ...
            <div className="relative p-8 md:p-12 flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              {!loading && !error && (
                <div className="md:ml-auto text-right">
                  ...
                  <div className="text-gray-400">Available tasker{filteredAndSortedCraftsmen.length !== 1 ? 's' : ''}</div>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {error && (
                <div className="flex flex-col items-center justify-center py-32 gap-4">
                  ...
                  <button onClick={() => window.location.reload()} className="mt-2 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors text-sm">Try again</button>
                </div>
              )}
              ...
              {!loading && !error && craftsmen.length === 0 && (
                <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
                  ...
                  <Link to="/browse-tasks" className="mt-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors font-semibold">Browse other categories</Link>
                </div>
              )}
